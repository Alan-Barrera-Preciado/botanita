...
for t = 0:dt:S
    ...
    x_Izq = F_K_Izq * x_Izq + G_K_Izq * u_Izq + 0.05* randn(2,1);
    x_Der = F_K_Der * x_Der + G_K_Der * u_Der + 0.05* randn(2,1);
    ...
    x_Izq_plot = [x_Izq_plot, x_Izq];
    x_Izq_hat_plot = [x_Izq_hat_plot, x_Izq_hat];
    u_Izq_plot = [u_Izq_plot, u_Izq];
    t_plot = [t_plot, t];
    ref_Izq_plot = [ref_Izq_plot, wr_Izq];
    err_Izq_plot = [err_Izq_plot, wr_Izq - x_Izq_hat(2)];

    x_Der_plot = [x_Der_plot, x_Der];
    x_Der_hat_plot = [x_Der_hat_plot, x_Der_hat];
    u_Der_plot = [u_Der_plot, u_Der];
...
figure;
hold on; grid on;
plot(t_plot, x_Izq_plot(2,:), 'LineWidth', 2);          % Velocidad real
plot(t_plot, x_Izq_hat_plot(2,:), 'LineWidth', 2);       % Estimación velocidad
plot(t_plot, ref_Izq_plot, 'k--', 'LineWidth', 2);      % Referencia
legend("Velocidad real", "Estimación", "Referencia");
xlabel("Tiempo (s)");
ylabel("Velocidad (rad/s)");
title("Seguimiento de referencia con LQR + Kalman - Motor Izquierdo");

figure;
plot(t_plot, u_Izq_plot, 'LineWidth', 2);
grid on;
xlabel("Tiempo (s)");
ylabel("Voltaje (V)");
title("Voltaje aplicado - Motor Izquierdo");

figure;
plot(t_plot, err_Izq_plot, 'LineWidth', 2);
grid on;
xlabel("Tiempo (s)");
ylabel("Error (rad/s)");
title("Error de seguimiento - Motor Izquierdo");

figure;
hold on; grid on;
plot(t_plot, x_Der_plot(2,:), 'LineWidth', 2);          % Velocidad real
plot(t_plot, x_Der_hat_plot(2,:), 'LineWidth', 2);       % Estimación velocidad
plot(t_plot, ref_Der_plot, 'k--', 'LineWidth', 2);      % Referencia
legend("Velocidad real", "Estimación", "Referencia");
xlabel("Tiempo (s)");
ylabel("Velocidad (rad/s)");
title("Seguimiento de referencia con LQR + Kalman - Motor Derecho");

figure;
plot(t_plot, u_Der_plot, 'LineWidth', 2);
grid on;
xlabel("Tiempo (s)");
ylabel("Voltaje (V)");
title("Voltaje aplicado - Motor Derecho");

figure;
plot(t_plot, err_Der_plot, 'LineWidth', 2);
grid on;
xlabel("Tiempo (s)");
ylabel("Error (rad/s)");
title("Error de seguimiento - Motor Derecho");